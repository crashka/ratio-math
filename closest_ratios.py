...
def closest_ratios(max_denom: int, midpack: bool = False) -> None:
    """Find closest, non-identical ratios with denominator less than ``max_denom``.  If
    ``midpack`` is specified as True, then skip computing the ratios at the extremities
    ...
    Note that this solution space is symmetrical around the midpoint of the numerator
    domain--that is, every pair of ratios, ``(n1/d1)`` and ``(n2/d2)``, has a companion
    pair, ``((d1-n1)/d1)`` and ``((d2-n2)/d2)``, with an identical difference.  Thus, we
    only need to explore the first half of possible numerators for any given denominator
    (none of the differences we are interested in spans the two halfs of the domain).
    """
    global operands, ratios, hi_denom
    assert max_denom > (4 if midpack else 2), "Degenerate case"
    assert max_denom > hi_denom, "Calling out of order"

    for denom in range(hi_denom, max_denom + 1):
        # we only have to run through the first half of possible numerators, skipping `1`
        # if `midpack` is specified
        for num in range(2 if midpack else 1, denom // 2 + 1):
            val = num / denom
...
def main() -> int | str:
    """Usage: python -m closest_ratios <max_denom> [<midpack> [<trace>]]

    where:

    - ``midpack`` (bool) - indicates whether to ignore ratios at the extremities (noting
    ...
    trace = False
    if len(sys.argv) < 2:
        return f"Missing arg(s)\n\n{main.__doc__}"
    max_denom = int(sys.argv[1])
    if len(sys.argv) > 2:
        midpack = bool_val(sys.argv[2])
        if len(sys.argv) > 3:
    ...
    start_denom = 5 if midpack else 3
    try:
        for n in range(start_denom, max_denom + 1):
            ratio1, ratio2, diff, is_mid = closest_ratios(n, midpack)
